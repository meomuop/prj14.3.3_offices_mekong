<?php
    ...
    // ------------------------------
    require_once ($CLASSES_PATH.'/hopdong/clsDuan.php');
    ...
    // --- Items sort
    if($vars['sort_me']==1){
        $obj->sortItem($vars['val'],$vars['duan_id']);
    }

    // --- Add - Edit
    if($vars['add_edit']==1):
        if (!isset($vars['duan_id']) || $vars['duan_id'] < 1) {
            $obj = new duan_class();
            $obj->readForm();
            if ((is_null($error)) || ($error == "")) {
                $obj->duan_date = date("Y-m-d");
                $obj->insertnew();
                unset($obj);
            }
        }else{
            ...
            if ((is_null($error)) || ($error == "")) {
                $obj->duan_date = date("Y-m-d");
                if ($obj->is_already_used($obj->tablename, "duan_id", $obj->duan_id))
                {
                    $obj->update();
                    unset($obj);
                }
    ...
    if($vars['edit_me']==1):
        $obj = new duan_class();
        $obj->getDBbyPkey($vars['duan_id']);
        if (!$obj->duan_id) redirect("?listDuan".$arg['arg']);
        $obj_info = (array)$obj;
    endif;

    // --- Condition : The row 71 got trouble in uesed --> can't findout the reason
    if($order_id == 2):
        $order_str = "duan_sort ASC";
    elseif($order_id == 1):
        $order_str = "duan_name ASC";
    else:
        $order_str = "duan_id DESC";
    endif;

    //$order_str = ($order_id == 1)?"duan_name":"duan_id DESC";

    // --- Condition
    $where = " 1 = 1";

    // --- Get data to view in homepage
    if (trim($vars['keyword'])) $where .= " AND duan_name regexp binary '".trim($vars['keyword'])."' ";
    ...
    $display = dirname(__FILE__)."/skin/B_Duan_tbl.tpl";
    ...
    if (isset($vars['activeAjax']))
        $smarty->display(dirname(__FILE__)."/skin/B_Duan_tbl.tpl");
    else
