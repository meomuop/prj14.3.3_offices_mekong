...
--------------------------------------------*/
	...
	$processurl = "?listHdbanHoadon&mod=hdban";
	...
	if($vars['add_edit']==1):
        if (!isset($vars['hoadon_id']) || $vars['hoadon_id'] < 1) {
            ...
            if ((is_null($error)) || ($error == "")) {
                ...
                $obj_trunghop = new hdbanHoadon_class();
	...
	if($vars['edit_me']==1):
        ...
        if (!$obj->hoadon_id) redirect("?listHdbanHoadon".$arg['arg']);
        $obj_info = (array)$obj;
	...
	$display = dirname(__FILE__)."/skin/B_HdbanHoadon_tbl.tpl";
	...
	$smarty->display(dirname(__FILE__)."/skin/B_HdbanHoadon_tbl.tpl");
	else
