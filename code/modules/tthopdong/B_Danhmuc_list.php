...
--------------------------------------------*/
 ...
 // --- Items sort
 if($vars['sort_me']==1){
 	$obj->sortItem($vars['val'],$vars['danhmuc_id']);
 }

 // --- Add - Edit
 if($vars['add_edit']==1):
	 if (!isset($vars['danhmuc_id']) || $vars['danhmuc_id'] < 1) {
		$obj = new danhmuc_class();
		$obj->readForm();
 ...
 if($vars['edit_me']==1):
	 $obj = new danhmuc_class();
	 $obj->getDBbyPkey($vars['danhmuc_id']);
	 if (!$obj->danhmuc_id) redirect("?listDanhmuc".$arg['arg']);
	 $obj_info = (array)$obj;
 endif;

 // --- Condition : The row 71 got trouble in uesed --> can't findout the reason
 if($order_id == 2):
 $order_str = "danhmuc_sort ASC";
 elseif($order_id == 1):
 $order_str = "danhmuc_name ASC";
 else:
 $order_str = "danhmuc_id DESC";
 endif;

 //$order_str = ($order_id == 1)?"danhmuc_name":"danhmuc_id DESC";
 
 // --- Condition
 $where = " 1 = 1";
 
 // --- Get data to view in homepage
 if (trim($vars['keyword'])) $where .= " AND danhmuc_name regexp binary '".trim($vars['keyword'])."' ";
 ...
 $display = dirname(__FILE__)."/skin/B_Danhmuc_tbl.tpl";
 ...
 if (isset($vars['activeAjax']))
 	$smarty->display(dirname(__FILE__)."/skin/B_Danhmuc_tbl.tpl");
 else
