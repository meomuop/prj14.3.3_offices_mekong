<?php
    ...
    $processurl = "?listDoitac&mod=hdmua";
    ...
    // --- Items sort
    if($vars['sort_me']==1){
        $obj->sortItem($vars['val'],$vars['doitac_id']);
    }

    // --- Add - Edit
    if($vars['add_edit']==1):
        if (!isset($vars['doitac_id']) || $vars['doitac_id'] < 1) {
            $obj = new doitac_class();
            $obj->readForm();
            if ((is_null($error)) || ($error == "")) {
                $obj->doitac_date = date("Y-m-d");
                $obj->insertnew();
                unset($obj);
            }
        }else{
            ...
            if ((is_null($error)) || ($error == "")) {
                $obj->doitac_date = date("Y-m-d");
                if ($obj->is_already_used($obj->tablename, "doitac_id", $obj->doitac_id))
                {
                    $obj->update();
                    unset($obj);
                }
    ...
    if($vars['edit_me']==1):
        $obj = new doitac_class();
        $obj->getDBbyPkey($vars['doitac_id']);
        if (!$obj->doitac_id) redirect("?listDoitac".$arg['arg']);
        $obj_info = (array)$obj;
    endif;

    // --- Condition : The row 71 got trouble in uesed --> can't findout the reason
    if($order_id == 2):
        $order_str = "doitac_sort ASC";
    elseif($order_id == 1):
        $order_str = "doitac_name ASC";
    else:
        $order_str = "doitac_id DESC";
    endif;

    //$order_str = ($order_id == 1)?"doitac_name":"doitac_id DESC";

    // --- Condition
    $where = " 1 = 1";

    // --- Get data to view in homepage
    if (trim($vars['keyword'])) $where .= " AND doitac_name regexp binary '".trim($vars['keyword'])."' ";
    ...
    $display = dirname(__FILE__)."/skin/B_Doitac_tbl.tpl";
    ...
    if (isset($vars['activeAjax']))
        $smarty->display(dirname(__FILE__)."/skin/B_Doitac_tbl.tpl");
    else
