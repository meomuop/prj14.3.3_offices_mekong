...
--------------------------------------------*/
	...
	// --- Condition
    $where = " 1 = 1";
    if ($vars['tungay']) 	$where .= " AND hdmua_ngayhd >= '".$vars['tungay']."'";
    if ($vars['denngay']) 	$where .= " AND hdmua_ngayhd <= '".$vars['denngay']."'";
    if ($vars['department_fil']) 	$where .= " AND department_id = '".$vars['department_fil']."'";
    if ($vars['tinhtrang_fil']) 	$where .= " AND hdmua_tinhtrang = '".$vars['tinhtrang_fil']."'";
    ...
    if($vars['exptype']==2):
        ...
        header("content-disposition: attachment;filename=Hopdong_Mua_".date('d-m-Y').".xls");
    elseif($vars['exptype']==1):
        header("Content-Type: application/vnd.ms-word");
        header("Expires: 0");
        header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
        header("content-disposition: attachment;filename=Hopdong_Mua_".date('d-m-Y').".doc");
    endif;
	...
	$display = dirname(__FILE__)."/skin/B_Hdmua_exp.tpl";
	...
	$smarty->display(dirname(__FILE__)."/skin/B_Hdmua_exp.tpl");
	else
