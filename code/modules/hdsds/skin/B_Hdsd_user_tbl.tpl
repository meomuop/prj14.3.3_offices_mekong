<form method="post" name="frmList" action="?listHdsd{$vars.arg}" id="frmList">
...
<table class="adminlist" width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
		<td style="font-family: Arial; color:#930; font-size: 16px; font-weight: bold;" width="30%">
        	<div align="left"><img src="../images/admin/module.png" align="absmiddle" height="30">&nbsp;Danh sách hướng dẫn sử dụng phần mềm</div>
        </td>			
	  	<td>
        	<div style="text-align: right; padding-bottom: 3px;">
                <div style="padding-bottom: 3px; ">
                    ...
                    <div style="float:right; padding-top:3px; text-align:right; padding-right: 20px;">Có tất cả {$total_num_result} hướng dẫn</div>
                </div>
            </div>
        </td>
	</tr>
...
<table width = "95%" border = "1" align="center" class="adminlist">
	<tr  bgcolor="#EEEEEE" >
		...
		<td align="center"><font color=#000000><B>Tên hướng dẫn</B></font></td>
	...
	<tr {if $smarty.section.pi.index is not div by 2} bgcolor="#F2FFEC" {/if} >
        ...
        <td align="left">
        	{if $per_edit eq 1}
            <a href="javascript:void(0)" onclick="window.open('?viewHdsd&id={$obj_list[pi]->hdsd_id}{$vars.arg}','HDSD','width=1150,height=1000,scrollbars=1,toolbar=0,location=0')" >{$obj_list[pi]->hdsd_name}</a>
            {else}{$obj_list[pi]->hdsd_name}{/if}
        </td>
                
        <td align="center">{$obj_list[pi]->hdsd_date|date_format:"%d-%m-%Y"}</td>
...
<div style="clear:both"><hdsd src='../images/admin/spacer.gif' height="10px" /></div>
