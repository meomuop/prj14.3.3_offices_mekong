...
--------------------------------------------*/
	...
	if($vars['add_edit']==1):
		if($vars['doc_note'] && $vars['doc_moved'] && $vars['user_id'] && $vars['doc_id']):
			$obj_doc -> updatePhanloai($vars['doc_note'],$vars['doc_moved'],$vars['user_id'],$vars['doc_id']);
		endif;
	endif;
	...
	// --- Display template
	$smarty->display(dirname(__FILE__)."/skin/B_Doc_phanloai.tpl");
?>