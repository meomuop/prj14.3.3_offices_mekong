...
--------------------------------------------*/
	...
	$order_arr = array(0 => "Chưa tham mưu", 1 => "Đã tham mưu", 2 => "Đã giải quyết", 3 => "Đã báo cáo", 4 => "Đã xác nhận");
	...
	// --- Bao cao van ban len lanh dao
	if ($vars['BCdoc_id']){
		$obj->UpdateBC($vars['BCdoc_id']);
	}
	...
	if($order_id == 0):
		$where .=" AND doc_traned = 2 and doc_active = 2";
	elseif($order_id == 1):
		$where .=" AND doc_traned = 1 and doc_active = 2";
	elseif($order_id == 2):
		$where .=" AND doc_traned = 1 and doc_active = 1";
    elseif($order_id == 3):
        $where .=" AND doc_traned = 1 and doc_active = 3";
    elseif($order_id == 4):
        $where .=" AND doc_traned = 1 and doc_active = 4";
	endif;
	...
	*/
	...
	$display = dirname(__FILE__)."/skin/B_Doc_tran_tbl.tpl";
	...
	$smarty->display(dirname(__FILE__)."/skin/B_Doc_tran_tbl.tpl");
	else
