...
<div id="doc_content_insign">
	<form method="post" name="frmList_Doc" action="?phoihopDoc{$vars.arg}" id="frmList_Doc">
    <!------------------------------------DANH SACH---------------------------------------------->
    <fieldset style="width:912px; border:1px solid #99bbe8; margin-left:3px; margin-top:5px; height:462px">
        ...
        <div style="float:left; height:404px; width:912px; overflow-y:scroll">
            ...
            <div style="float:left">
                ...
                <div class="{$class_td}" style="width:70px" title="{$obj_list[pi]->doc_code}">
                ...
                <div class="{$class_td}" style="width:370px" title="{$obj_list[pi]->doc_desc}">
                    ...
                    <div style="float:left; width:325px; white-space:nowrap">&nbsp;<a href="javascript: void(0);" onclick="show_thongtin_vanban({$obj_list[pi]->doc_id})" >{$obj_list[pi]->doc_desc|str_string_cut:"88":"1"}</a></div>
                    ...
                    {if $user_level eq 5 or $user_level eq 6 or $user_level eq 7 or $user_level eq 8 or $user_level eq 3}<a href='javascript: void(0);' onclick="show_list_options({$count},{$total_num_result})">
                    ...
                    <div id="extra_{$count}" style="position:absolute; display:none; height:auto; width:140px; margin:2px 209px; border:1px solid #8ea4c1; border-right:8ea4c1; background-color:#dfe8f6">
                        ...
                        {if $user_level eq 6 or $user_level eq 7}
                        <div style="float:left; width:140px; padding:0 0 0 3px">
                        	{php}
                                $sql="select * from tbl_addusers where doc_id =".$this->_tpl_vars['obj_list'][$this->_sections['pi']['index']]->doc_id."";
                            ...
                            {/php}
                               - <a href="javascript: void(0)" onclick="javascript: show_phoihop({$order_id},{$obj_list[pi]->doc_id})">Chuyển phối hợp</a>
                            {php}
                                endif;
                            {/php}
                        </div>
...
<script language="javascript">	
	...
	function show_phoihop(order_id,doc_id){
		$('#div_phoihop').css({'display':'block','z-index':'2010'});
		$("#list_phoihop_cont").load("?listAddUser&mod=docs"+"&order_id="+order_id+"&doc_id="+doc_id);
	}
	...
	function uncheck_fns_Doc(field,val,id){
		$("#doc_content_main").load("?phoihopDoc&mod=docs&checkUncheck="+val+"&setfield="+field+"&checkUncheckID="+id);
	}
	
	function sort_me_Doc(id,val){
		$("#doc_content_main").load("?phoihopDoc&mod=docs&sort_me=1&doc_id="+id+"&val="+val);
	}
	...
	function search_me_Doc(){
		...
		$.post('?phoihopDoc&mod=docs&search_me=1',
			{
	...
	function export_doc(exptype){
		...
		if(exptype==3){
			...
			$('#doc_content_main').fadeOut('fast').load('index.php?phoihopDoc&mod=docs').fadeIn("fast");
	...
	function edit_me_Doc(id){
		$("#doc_content_main").load("?phoihopDoc&mod=docs&edit_me=1&doc_id="+id);
	}
