...
<div id="doc_content_insign">
	<form method="post" name="frmList_Doc" action="?finishDoc{$vars.arg}" id="frmList_Doc">
    <!------------------------------------DANH SACH---------------------------------------------->
        ...
        <div style="float:left; height:404px; width:912px; overflow-y:scroll">
            ...
            <div style="float:left">
                ...
                <div class="{$class_td}" style="width:370px" title="{$obj_list[pi]->doc_desc}">
                    ...
                    {if $user_level eq 10}<a href='javascript: void(0);' onclick="show_list_options({$count},{$total_num_result})">
                    ...
                    <div id="extra_{$count}" style="position:absolute; display:none; height:auto; width:140px; margin:2px 209px; border:1px solid #8ea4c1; border-right:8ea4c1; background-color:#dfe8f6">
                        ...
                        {else}
                        <div style="float:left; width:140px; padding:0 0 0 3px">
                            - <a href="javascript: void(0)" onclick="javascript: show_giaiquyet({$obj_list[pi]->doc_id})">Phục hồi</a>
                        </div>
                        {/if}
                        <div style="float:left; width:140px; padding:0 0 0 3px">
                            - <a href="javascript: void(0)" onclick="javascript: in_phieu_giaoviec({$obj_list[pi]->doc_id})">In phiếu giao việc</a>
                        </div>
                        <div style="float:left; width:140px; padding:0 0 0 3px">
...
<script language="javascript">	
    ...
    function in_phieu_giaoviec(doc_id){
        window.open("?printPGV&mod=docs&doc_id="+doc_id);
    }
	...
	function uncheck_fns_Doc(field,val,id){
		$("#doc_content_main").load("?finishDocAll&mod=docs&checkUncheck="+val+"&setfield="+field+"&checkUncheckID="+id);
	}
	
	function sort_me_Doc(id,val){
		$("#doc_content_main").load("?finishDocAll&mod=docs&sort_me=1&doc_id="+id+"&val="+val);
	}
	...
	function search_me_Doc(){
		...
		$.post('?finishDocAll&mod=docs&search_me=1',
			{
	...
	function export_doc(exptype){
		...
		if(exptype==3){
			...
			$('#doc_content_main').fadeOut('fast').load('index.php?finishDocAll&mod=docs').fadeIn("fast");
	...
	function edit_me_Doc(id){
		$("#doc_content_main").load("?finishDocAll&mod=docs&edit_me=1&doc_id="+id);
	}
