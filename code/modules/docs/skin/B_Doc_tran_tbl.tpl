...
<div id="doc_content_insign">
	<form method="post" name="frmList_Doc" action="?tranDoc{$vars.arg}" id="frmList_Doc">
    <!------------------------------------DANH SACH---------------------------------------------->
    <fieldset style="width:912px; border:1px solid #99bbe8; margin-left:3px; margin-top:5px; height:462px">
        <legend class="legend_list" style="width:865px">
            ...
            <div style="width:260px; float:left; font-weight:normal; position:absolute; display:none; margin:20px 0 0 604px; background-color:#ccd8e7; border:1px solid #99bbe8; border-top:none; z-index:2012; padding-top: 5px" id="div_filter">
                ...
                <div style="float:left; width:76px; margin-bottom:5px">- Vùng dữ liệu: </div>
                <div style="float:left; width:150px; margin-bottom:5px">
                    <select name="data_zone" id="data_zone" class="select_seek">
                        <option value="1">Dữ liệu chung</option>
                        <option value="2">Dữ liệu của ban</option>
                    </select>
                </div>
                <div style="float:left; width:76px; margin-bottom:5px">- Kiểu báo cáo: </div>
                <div style="float:left; width:150px; margin-bottom:5px">
                    <select name="report_type" id="report_type" class="select_seek">
                        <option value="1">Báo cáo chung</option>
                        <option value="2">Báo cáo chậm</option>
                    </select>
                </div>
            </div>
        </legend>
        ...
        <div style="float:left; height:404px; width:912px; overflow-y:scroll">
            ...
            <div style="float:left">
                ...
                <div class="{$class_td}" style="width:369px" title="{$obj_list[pi]->doc_desc}">
                	{math a=$smarty.section.pi.index b=1 equation="a+b" assign=count}
                    <div style="float:left; width:345px; white-space:nowrap">&nbsp;<a href="javascript: void(0);" onclick="show_thongtin_vanban({$obj_list[pi]->doc_id})" >{$obj_list[pi]->doc_desc|str_string_cut:"88":"1"}</a></div>
                    ...
                    {if $user_level eq 3}<a href='javascript: void(0);' onclick="show_list_options({$count},{$total_num_result})">
                    <img src="{$css_path}extra_icon.png" width="14" height="14" border="0" class="img_all"></a>{/if}
                    </div>
                    <div id="extra_{$count}" style="position:absolute; display:none; height:auto; width:140px; margin:2px 209px; border:1px solid #8ea4c1; border-right:8ea4c1; background-color:#dfe8f6">
                        ...
                        <div style="float:left; width:140px; padding:0 0 0 3px">
                            ...
                            {if $user_list_4[ui]->user_level eq 3}
                                 - <a href="javascript: void(0)" onclick="javascript: show_phanloai({$obj_list[pi]->doc_id})">Tham mưu</a>
                            {/if}
                            {/section}
                        </div>
                        {/if}
                        {if $obj_list[pi]->doc_active eq 1}
                            <div style="float:left; width:140px; padding:0 0 0 3px">
                                - <a href="javascript: void(0)" onclick="javascript: bao_cao_van_ban({$obj_list[pi]->doc_id})">Báo cáo lãnh đạo</a>
                            </div>
                        {/if}
                        <div style="float:left; width:140px; padding:0 0 0 3px">
                ...
                <div class="{$class_td}" style="width:132px" title="{$obj_list[pi]->unit_name}">
...
<script language="javascript">	
	...
	function show_list_file(doc_id){
		...
		$("#list_file_cont").load("?listDocFile&mod=docs&tran_doc=1"+"&doc_id="+doc_id);
	}
		
	function show_phanloai(doc_id){
		document.getElementById('list_phanloai').style.display='block';
		var arg = document.getElementById('arg').value;
		$("#list_phanloai_cont").load("?Phanloai&arg="+arg+"&doc_id="+doc_id);
	}

    function bao_cao_van_ban(doc_id){
        var arg = document.getElementById('arg').value;
        $("#doc_content_main").load("?tranDoc&arg="+arg+"&BCdoc_id="+doc_id);
    }
	...
	function uncheck_fns_Doc(field,val,id){
		$("#doc_content_main").load("?tranDoc&mod=docs&checkUncheck="+val+"&setfield="+field+"&checkUncheckID="+id);
	}
	
	function sort_me_Doc(id,val){
		$("#doc_content_main").load("?tranDoc&mod=docs&sort_me=1&doc_id="+id+"&val="+val);
	}
	...
	function search_me_Doc(){
		...
		$.post('?tranDoc&mod=docs&search_me=1',
			{
	...
	function export_doc(exptype){
        ...
        var data_zone 	    = document.getElementById('data_zone').value;
        ...
        if(report_type==1){
            $url_exp = 'expDocReport';
        }else{
            $url_exp = 'expBCVBDenCham';
        }
		
		if(exptype==3){
			document.getElementById('div_doc_report').style.display='block';
			$.post('?'+$url_exp+'&mod=docs&exptype='+exptype,
				{
					tungay:tungay,
					denngay:denngay,
					soden_tu:soden_tu,
			...
			$('#doc_content_main').fadeOut('fast').load('index.php?tranDoc&mod=docs').fadeIn("fast");
	...
	function edit_me_Doc(id){
		$("#doc_content_main").load("?tranDoc&mod=docs&edit_me=1&doc_id="+id);
	}
