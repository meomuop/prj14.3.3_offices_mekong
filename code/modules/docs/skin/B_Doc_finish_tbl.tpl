        ...
        <div style="float:left; height:404px; width:912px; overflow-y:scroll">
            ...
            <div style="float:left">
                ...
                <div class="{$class_td}" style="width:370px" title="{$obj_list[pi]->doc_desc}">
                    ...
                    {if $user_level eq 8}<a href='javascript: void(0);' onclick="show_list_options({$count},{$total_num_result})">
...
<script language="javascript">	
	...
	function uncheck_fns_Doc(field,val,id){
		$("#doc_content_main").load("?finishDoc&mod=docs&checkUncheck="+val+"&setfield="+field+"&checkUncheckID="+id);
	}
	
	function sort_me_Doc(id,val){
		$("#doc_content_main").load("?finishDoc&mod=docs&sort_me=1&doc_id="+id+"&val="+val);
	}
	...
	function search_me_Doc(){
		...
		$.post('?finishDoc&mod=docs&search_me=1',
			{
	...
	function export_doc(exptype){
		...
		if(exptype==3){
			...
			$('#doc_content_main').fadeOut('fast').load('index.php?finishDoc&mod=docs').fadeIn("fast");
	...
	function edit_me_Doc(id){
		$("#doc_content_main").load("?finishDoc&mod=docs&edit_me=1&doc_id="+id);
	}
