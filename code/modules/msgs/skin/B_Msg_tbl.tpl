...
<div id="msg_content_insign">
	<form method="post" name="frmList_Msg" action="?listMsg{$vars.arg}" id="frmList_Msg">
    <!------------------------------------THAO TAC TIN NHAN---------------------------------------------->
    ...
    <fieldset style="width:380px; border:1px solid #99bbe8; margin-left:3px; margin-top:5px; height:272px">
        ...
        <div style="float:left; height:218px; width:380px; overflow-y:scroll">
            ...
            <div class="{$class_td}" style="width:259px">
            	<div style="float:left; width:230px; white-space:nowrap"><a href="javascript:void(0)" onclick="show_noidung_msg({$obj_list[mi]->msg_id},'listMsg')">&nbsp;{$obj_list[mi]->msg_cont|str_string_cut:50:1}</a></div>
                <div style="float:right; margin:2px; height:21px; width:25px; text-align:center; background-color:#dfe8f6" title="Xóa">
                <a href="javascript: void(0)" onclick="javascript: delete_msg({$obj_list[mi]->msg_id})">
...
<script language="javascript">	
	...
	function search_me_Msg(){
		...
		$.post('?listMsg&mod=msgs&outbox=1',
			{keyword:keyword},
			function(data){
	...
	function delete_msg(delStr){
		...
		$.ajax({
			type: "POST",
			url: 'index.php?listMsg&mod=msgs&outbox=1',
			data: dataString,
	...
	$("#a_ref_Msg").click( function () { 
			...
			$('#msg_content_main').fadeOut('fast').load('?listMsg&mod=msgs&outbox=1').fadeIn("fast");
	});
