...
<div id="msg_content_insign">
	<form method="post" name="frmList_Msg" action="?listMsgRac{$vars.arg}" id="frmList_Msg">
    <!------------------------------------MENU SMS---------------------------------------------->
    ...
    <fieldset style="width:380px; border:1px solid #99bbe8; margin-left:3px; margin-top:5px; height:272px">
        ...
        <div style="float:left; height:218px; width:380px; overflow-y:scroll">
            ...
            <div class="{$class_td}" style="width:259px;">
                ...
                {/php}
                <a href="javascript:void(0)" onclick="show_noidung_msg({$obj_list[mi]->msg_id},'listMsgRac')">
                <font color="{php}echo $color{/php}">&nbsp;{$obj_list[mi]->msg_cont|str_string_cut:50:1}</font></a>
                </div>
                ...
                <div id="extra_msg_{$count}" class="{$class_td}" style="position:absolute; display:none; height:auto; width:180px; margin:22px 60px; border-bottom:1px solid #8ea4c1; border-right:1px solid #8ea4c1; border-left:1px solid #8ea4c1">
                    ...
                    <div style="float:left; width:180px; padding:1px">
                        ...
                        <div style="float:left; width:70px; padding:1px">
                             &bull; <a href="javascript: void(0)" onclick="javascript: update_thuong({$obj_list[mi]->msg_id})">Tin thường</a>
                        </div>
...
<script language="javascript">	
	...
	function search_me_Msg(){
		...
		$.post('?listMsgRac&mod=msgs&wastemsg=1',
			{keyword:keyword},
			function(data){
	...
	function update_quantrong(msg_id){
		...
		$('#msg_content_main').fadeOut('fast').load('index.php?listMsgRac&mod=msgs&wastemsg=1').fadeIn("fast");	
	}
	
	function update_thuong(msg_id){
		$('#msg_content_main').load('index.php?listMsgUser&mod=msgs&update_thuong=1&msg_id='+msg_id).fadeIn("fast");
		$('#msg_content_main').hide();
		$('#msg_content_main').fadeOut('fast').load('index.php?listMsgRac&mod=msgs&wastemsg=1').fadeIn("fast");	
	}
	
	function loc_dl(msg_type){
		var msg_name_fs = document.getElementById('msg_name_fs').value;		
		$.post('?listMsgRac&mod=msgs&msg_type='+msg_type,
			{msg_name_fs:msg_name_fs},
			function(data){
				$('#msg_content_main').html(data);
				$("#msg_content_main").show();
			}
	...
	function show_edit_form(msg_id,msg_type){
		$('#add_edit_msg').css({'display':'block','z-index':'2010'});
		$('#list_add_edit_msg_cont').load('index.php?addEditMsg&mod=msgs&edit_me=1&msg_id='+msg_id).fadeIn("fast");	
	}
			
	$("#a_ref_Msg").click( function () { 
		var msg_type = document.getElementById('msg_type').value;
		$('#msg_content_main').hide();
		$('#msg_content_main').fadeOut('fast').load('index.php?listMsgRac&mod=msgs').fadeIn("fast");
	});
</script>
{/literal}