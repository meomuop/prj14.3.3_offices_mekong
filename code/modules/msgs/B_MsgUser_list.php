...
- Created by Le Anh Van - anhvan3103@gmail.com
	...
	if ($vars['dlStr']){
		$obj_msgUser->removebyFK($vars['dlStr']);
	}
	
	if($vars['update_quantrong']==1):
		$obj_msgUser->updateType((int)$vars['msg_id'],$_SESSION['user_id'],1);
	endif;
	if($vars['update_thuong']==1):
		$obj_msgUser->updateType((int)$vars['msg_id'],$_SESSION['user_id'],0);
	endif;
	if($vars['update_rac']==1):
		$obj_msgUser->updateType((int)$vars['msg_id'],$_SESSION['user_id'],2);
	endif;
	...
	$where .= " AND msg_id in(select msg_id from tbl_msgusers where user_id = ".$_SESSION['user_id']." and msgUser_type = 0)";
	$obj = new msg_class();
	...
	$display = dirname(__FILE__)."/skin/B_MsgUser_tbl.tpl";
	...
	$smarty->display(dirname(__FILE__)."/skin/B_MsgUser_tbl.tpl");
	else
