...
--------------------------------------------*/
	...
	// --- Add - Edit
	if($vars['add_edit']==1):
	 if (!isset($vars['vttb_cat_id']) || $vars['vttb_cat_id'] < 1) {
		$obj = new vttb_cat_class();
		$obj->readForm();
		if ((is_null($error)) || ($error == "")) {
			$obj->vttb_cat_date = date("Y-m-d");
			$obj->vttb_cat_ununicode = fnsUnUnicode($vars['vttb_cat_name']);
		...
		if ((is_null($error)) || ($error == "")) {
			...
			if ($obj->is_already_used($obj->tablename, "vttb_cat_id", $obj->vttb_cat_id))
			{
				$obj->update();
				unset($obj);
			}
	...
	if($vars['edit_me']==1):
	 $obj = new vttb_cat_class();
	 $obj->getDBbyPkey($vars['vttb_cat_id']);
	 $obj_info = (array)$obj;
	endif;
	...
	// --- Display template
	$smarty->display(dirname(__FILE__)."/skin/B_Vttb_cat_add_edit.tpl");
?>