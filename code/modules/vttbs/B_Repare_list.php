...
--------------------------------------------*/
	...
	// ------------------------------
	include ($CLASSES_PATH.'clsVttbRepares.php');
	include ($CLASSES_PATH.'clsVttbs.php');
	
	// --- Class is used in this page
	$obj = new vttb_repare_class();
	...
	$processurl = "?listRepare&mod=vttbs";
	...
	// --- Add - Edit
	if($vars['add_edit']==1):
	 if (!isset($vars['vttb_repare_id']) || $vars['vttb_repare_id'] < 1) {
		$obj = new vttb_repare_class();
		$obj->readForm();
		if ((is_null($error)) || ($error == "")) {
			$obj->vttb_repare_cost = str_replace('.','',$vars['vttb_repare_cost']);
		...
		$obj->readForm();
		if ((is_null($error)) || ($error == "")) {
			if ($obj->is_already_used($obj->tablename, "vttb_repare_id", $obj->vttb_repare_id)){
				$obj->vttb_repare_cost = str_replace('.','',$vars['vttb_repare_cost']);
				$obj->user_id = $_SESSION["user_id"];
				$obj->update();
	...
	if($vars['edit_me']==1):
	 $obj = new vttb_repare_class();
	 $obj->getDBbyPkey($vars['vttb_repare_id']);
	 //if (!$obj->vttb_repare_id) redirect("?VttbRepare".$arg['arg']);
	 $obj_info = (array)$obj;
	endif;
	
	// --- Condition : The row 71 got trouble in uesed --> can't findout the reason
	if($order_id == 2):
		$order_str = "vttb_repare_sort ASC";
	elseif($order_id == 1):
		$order_str = "vttb_repare_name ASC";
	else:
		$order_str = "vttb_repare_id DESC";
	endif;
	
	//$order_str = ($order_id == 1)?"vttb_repare_name":"vttb_repare_id DESC";
	...
	$obj = new vttb_repare_class();
	$obj_list = $obj->getDBList(" $where", "vttb_repare_id DESC", FALSE, $limit);
	...
	$display = dirname(__FILE__)."/skin/B_Repare_tbl.tpl";
	...
	if (isset($vars['activeAjax']))
		$smarty->display(dirname(__FILE__)."/skin/B_Repare_tbl.tpl");
	else
