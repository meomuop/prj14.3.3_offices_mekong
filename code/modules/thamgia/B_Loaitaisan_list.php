...
--------------------------------------------*/
 ...
 $order_arr = array(0 => "Thứ tự cập nhật", 1 => "Theo tên loại tài sản", 2 => "Theo thứ  tự");
 ...
 $processurl = "?listLoaitaisan&mod=taisans";
 ...
 $aa=array();
 $bb=array();
 $counta=count($vars['loaitaisan_sort']); 	
 $countb=count($vars['loaitaisan_id1']);
 if ($counta>0 and $countb>0) 
 { 
	for ($i=0;$i<$counta;$i++) 
	{ 
		$aa['maxe']=$vars['loaitaisan_sort'][$i]; 
		$bb['maxe1']=$vars['loaitaisan_id1'][$i];
 ...
 if($order_id == 2):
 $order_str = "loaitaisan_sort ASC";
 elseif($order_id == 1):
 $order_str = "loaitaisan_name ASC";
 else:
 $order_str = "loaitaisan_id DESC";
 endif;
 
 //$order_str = ($order_id == 1)?"loaitaisan_name":"loaitaisan_id DESC";
 
 // --- Condition
 $where = " 1 = 1 and taisan_id = ".$vars['taisan_id'];
 
 // --- Get data to view in homepage
 if (trim($vars['KeyWord'])) $where .= " AND loaitaisan_name LIKE '%".trim($vars['KeyWord'])."%' ";
 ...
 $display = dirname(__FILE__)."/skin/B_Loaitaisan_tbl.tpl";
 ...
 if (isset($vars['activeAjax']))
 	$smarty->display(dirname(__FILE__)."/skin/B_Loaitaisan_tbl.tpl");
 else
