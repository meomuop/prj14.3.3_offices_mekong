<form method="post" name="frmList" action="?listLoaitaisan{$vars.arg}" id="frmList">
...
<table class="adminlist" width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
		<td style="font-family: Arial; color:#930; font-size: 16px; font-weight: bold;" width="30%">
        	<div align="left"><img src="../images/admin/module.png" align="absmiddle" height="30">&nbsp;Danh sách loại tài sản</div>
        </td>			
	  	<td>
        	<div style="text-align: right; padding-bottom: 3px;">
                <div style="padding-bottom: 3px; ">
                    ...
                    <div style="float:right; padding-top:3px; text-align:right; padding-right: 20px;">Có tất cả {$total_num_result} loại tài sản</div>
                </div>
            </div>
        </td>
	</tr>
...
<table width = "95%" border = "1" align="center" class="adminlist">
	<tr  bgcolor="#EEEEEE" >
		...
		<td align="center"><font color=#000000><B>Tên loại tài sản</B></font></td>
	...
	<tr {if $smarty.section.pi.index is not div by 2} bgcolor="#F2FFEC" {/if} >
        ...
        <td align="left">
        	{if $per_edit eq 1}
            <a href="?editLoaitaisan&id={$obj_list[pi]->loaitaisan_id}{$vars.arg}" >{$obj_list[pi]->loaitaisan_name}</a>
            {else}{$obj_list[pi]->loaitaisan_name}{/if}
        </td>
        
        <td align="center">{$obj_list[pi]->loaitaisan_standfor}</td>
        
        <td align="center">{$obj_list[pi]->loaitaisan_date|date_format:"%d-%m-%Y"}</td>
		        
        <td width="16" ><div align="center">
        	{if $per_act eq 1}
         	<select name="loaitaisan_sort[]" style="width:40px">
            {section name=i loop=$total_num_result}
            	<option value="{$smarty.section.i.index}" {if $smarty.section.i.index eq $obj_list[pi]->loaitaisan_sort} selected="selected"{/if}>{$smarty.section.i.index}</option>
            {/section}
            </select>
            <input type="hidden" name="loaitaisan_id1[]" value="{$obj_list[pi]->loaitaisan_id}">
            {else}-{/if}
        </div></td>
        
        <td width="16" ><div align="center">
        	{if $per_act eq 1}
            {if $obj_list[pi]->loaitaisan_active eq 1}
                <a href='?listLoaitaisan&checkUncheck=2&setfield=loaitaisan_active&checkUncheckID={$obj_list[pi]->loaitaisan_id}{$vars.arg}'>
                <img src="../images/admin/tick.png" width="10" height="10" border="0"></a>
            {else}
                <a href='?listLoaitaisan&checkUncheck=1&setfield=loaitaisan_active&checkUncheckID={$obj_list[pi]->loaitaisan_id}{$vars.arg}'>
        ...
        <td width="17" valign="middle" align="center">
        	{if $per_edit eq 1}
            <a href='?editLoaitaisan&id={$obj_list[pi]->loaitaisan_id}{$vars.arg}'>
            <img src="../images/admin/b_edit.png" width="16" height="16" border="0" title="Sửa"></a>
            {else}-{/if}
        </td>
		<td width="16" valign="middle" align="center">
        	{if $per_del eq 1}
            <a href='#'  onClick="delItems('{$processurl}', {$obj_list[pi]->loaitaisan_id});">
            <img src="../images/admin/b_drop.png" width="16" height="16" border="0" title="Xóa"></a>
            {else}-{/if}
        </td>	
		<td style="text-align:center">
        	<input type="checkbox" name="chkid" value="{$obj_list[pi]->loaitaisan_id}" onclick="docheckone();">
        </td>
	</tr>
	{/section}
	<tr>
		<td colspan="8" style="text-align:left">
			...
			<div style="float:right">
                ...
                &nbsp;
                {if $per_add eq 1}
				<input type="button" name="Submit2" value="Thêm mới" onClick="document.location='?addLoaitaisan{$vars.arg}'">
                {else}
                <input type="button" name="Submit2" value="Thêm mới" onClick="document.location='?addLoaitaisan{$vars.arg}'" disabled>
                {/if}
			</div>
        </td>
		<td style="text-align:center">
