<form method="post" name="frmList" action="?listTaisan{$vars.arg}" id="frmList">
...
<table class="adminlist" width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
		<td style="font-family: Arial; color:#930; font-size: 16px; font-weight: bold;" width="30%">
        	<div align="left"><img src="../images/admin/module.png" align="absmiddle" height="30">&nbsp;Danh sách tài sản</div>
        </td>			
	  	<td>
        	<div style="text-align: right; padding-bottom: 3px;">
                <div style="padding-bottom: 3px; ">
                    ...
                    <div style="float:right; padding-top:3px; text-align:right; padding-right: 20px;">Có tất cả {$total_num_result} tài sản</div>
                </div>
            </div>
        </td>
	</tr>
...
<table width = "95%" border = "1" align="center" class="adminlist">
	<tr  bgcolor="#EEEEEE" >
		...
		<td align="center"><font color=#000000><B>Tên tài sản</B></font></td>
        <td width="10%" align="center" ><font color=#000000><B>Viết tắt</B></font></td>
        <td width="10%" align="center" ><font color=#000000><B>Kiểu</B></font></td>
        <td width="10%" align="center" ><font color=#000000><B>Chi tiết</B></font></td>
        <td width="10%" align="center" ><font color=#000000><B>Ngày nhập</B></font></td>
	...
	<tr {if $smarty.section.pi.index is not div by 2} bgcolor="#F2FFEC" {/if} >
        ...
        <td align="left">
        	{if $per_edit eq 1}
            <a href="?editTaisan&id={$obj_list[pi]->taisan_id}{$vars.arg}" >{$obj_list[pi]->taisan_name}</a>
            {else}{$obj_list[pi]->taisan_name}{/if}
        </td>
        
        <td align="left">{$obj_list[pi]->taisan_standfor}</td>
        
        <td align="center">
        {if $obj_list[pi]->taisan_type eq 1}Thẩm định giá
        {elseif $obj_list[pi]->taisan_type eq 2}TĐG + BĐG
        {else}Bán đấu giá
        {/if}
        </td>
        
        <td align="center">
            <a href="?listLoaitaisan&taisan_id={$obj_list[pi]->taisan_id}&mod=taisans" >
            <img src="../images/admin/b_view.png" width="20" height="20" border="0"></a>
        </td>
        
        <td align="center">{$obj_list[pi]->taisan_date|date_format:"%d-%m-%Y"}</td>
		        
        <td width="16" ><div align="center">
        	{if $per_act eq 1}
         	<select name="taisan_sort[]" style="width:40px">
            {section name=i loop=$total_num_result}
            	<option value="{$smarty.section.i.index}" {if $smarty.section.i.index eq $obj_list[pi]->taisan_sort} selected="selected"{/if}>{$smarty.section.i.index}</option>
            {/section}
            </select>
            <input type="hidden" name="taisan_id1[]" value="{$obj_list[pi]->taisan_id}">
            {else}-{/if}
        </div></td>
        
        <td width="16" ><div align="center">
        	{if $per_act eq 1}
            {if $obj_list[pi]->taisan_active eq 1}
                <a href='?listTaisan&checkUncheck=2&setfield=taisan_active&checkUncheckID={$obj_list[pi]->taisan_id}{$vars.arg}'>
                <img src="../images/admin/tick.png" width="10" height="10" border="0"></a>
            {else}
                <a href='?listTaisan&checkUncheck=1&setfield=taisan_active&checkUncheckID={$obj_list[pi]->taisan_id}{$vars.arg}'>
	...
	<tr>
		<td colspan="10" style="text-align:left">
			...
			<div style="float:right">
                ...
                &nbsp;
                {if $per_add eq 1}
				<input type="button" name="Submit2" value="Thêm mới" onClick="document.location='?addTaisan{$vars.arg}'">
                {else}
                <input type="button" name="Submit2" value="Thêm mới" onClick="document.location='?addTaisan{$vars.arg}'" disabled>
                {/if}
			</div>
        </td>
		<td style="text-align:center">
