...
--------------------------------------------*/
 ...
 $order_arr = array(0 => "Thứ tự cập nhật", 1 => "Theo tên cổng", 2 => "Theo thứ  tự");
 ...
 // --- Items sort
 if($vars['sort_me']==1){
 	$obj->sortItem($vars['val'],$vars['port_id']);
 }
 
 // --- Add - Edit
 if($vars['add_edit']==1):
	 if (!isset($vars['port_id']) || $vars['port_id'] < 1) {
		$obj = new port_class();
		$obj->readForm();
		if ((is_null($error)) || ($error == "")) {
			$obj->port_date = date("Y-m-d");
			$obj->insertnew();
			unset($obj);
		}
	 }else{
		...
		if ((is_null($error)) || ($error == "")) {
			$obj->port_date = date("Y-m-d");
			if ($obj->is_already_used($obj->tablename, "port_id", $obj->port_id))
			{
				$obj->update();
				unset($obj);
			}
 ...
 if($vars['edit_me']==1):
	 $obj = new port_class();
	 $obj->getDBbyPkey($vars['port_id']);
	 if (!$obj->port_id) redirect("?listPort".$arg['arg']);
	 $obj_info = (array)$obj;
 endif;
 
 // --- Condition : The row 71 got trouble in uesed --> can't findout the reason
 if($order_id == 2):
 $order_str = "port_sort ASC";
 elseif($order_id == 1):
 $order_str = "port_name ASC";
 else:
 $order_str = "port_id DESC";
 endif;
 
 //$order_str = ($order_id == 1)?"port_name":"port_id DESC";
 
 // --- Condition
 $where = " 1 = 1";
 
 // --- Get data to view in homepage
 if(isset($vars['keyword'])) $where .= " AND port_name regexp binary '".trim($vars['keyword'])."' ";
 ...
 $display = dirname(__FILE__)."/skin/B_port_tbl.tpl";
 ...
 if (isset($vars['activeAjax']))
 	$smarty->display(dirname(__FILE__)."/skin/B_port_tbl.tpl");
 else
 	$smarty->display(dirname(__FILE__)."/skin/B_port_list.tpl");
?>