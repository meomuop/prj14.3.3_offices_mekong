...
<div id="phanloai_content_insign">
    ...
    <fieldset style="width:523px; border:1px solid #99bbe8; margin-left:3px; margin-top:5px">
    	...
    	<div style="float:left; width:513px" class="text_all">
            ...
            <div style="float:left; width:78px" title="{$vars.docCat_id}">{$vars.docCat_id|str_string_cut:"18":"1"};</div>
        ...
        <div style="float:left; width:513px" class="text_all">
            ...
            <div style="float:left; width:78px">
            	{section name=ii loop=$obj_list_important}
                    {if $obj_list_important[ii]->important_id eq $vars.important_id}
                    {$obj_list_important[ii]->important_name};
                    {/if}
                {/section}
            </div>
            <div style="float:left; width:46px">Độ mật:</div>
            <div style="float:left; width:88px">
            	{section name=si loop=$obj_list_secret}
                    {if $obj_list_secret[si]->secret_id eq $vars.secret_id}
                    {$obj_list_secret[si]->secret_name};
                    {/if}
                {/section}
            </div>
            <div style="float:left; width:55px">Người ký:</div>
            <div style="float:left; width:185px">{$vars.signPer_id}</div>
        </div>
    </fieldset>
            ...
            <div style="float:left">
                ...
                <div class="{$class_td}" style="width:98px; white-space:nowrap">
                	{section name=ui loop=$obj_list_user}
                    	{if $obj_list_user[ui]->user_id eq $obj_list_docfile[pi]->user_id}
