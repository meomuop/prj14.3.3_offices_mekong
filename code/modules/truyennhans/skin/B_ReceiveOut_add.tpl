...
<div id="truyenvb_content_insign">
    ...
    <fieldset style="width:555px; border:1px solid #99bbe8; margin-left:3px; margin-top:5px">
        ...
        <input type="hidden" value="{$signPer_info.user_fullname}" id="signPer_id" name="signPer_id"/>
        <input type="hidden" value="{$doc_info.docOut_id}" id="docout_id" name="docout_id"/>
        <input type="hidden" value="{$doc_info.department_id}" id="department_id" name="department_id"/>
        <input type="hidden" value="{$doc_info.docField_id}" id="docField_id" name="docField_id"/>
        <input type="hidden" value="{$doc_info.docOut_code}" id="doc_code" name="doc_code"/>
        <input type="hidden" value="{if $doc_info.important_id neq 0}{$doc_info.important_id}{else}0{/if}" id="important_id" name="important_id"/>
        <input type="hidden" value="{if $doc_info.secret_id neq 0}{$doc_info.secret_id}{else}0{/if}" id="secret_id" name="secret_id"/>
        <input type="hidden" value="{$docCat_info.docCat_name}" id="docCat_id" name="docCat_id"/>
        <input type="hidden" value="{$doc_info.docOut_desc}" id="doc_desc" name="doc_desc"/>
    	...
    	<div style="float:left; width:545px; height:35px; overflow-x:scroll; white-space:nowrap" class="text_all">
        	Số K.Hiệu: {$doc_info.docOut_code}; 
            Các tệp tin:
            ...
            {section name=pi loop=$obj_list_docoutfile}
                ...
                {$obj_list_docoutfile[pi]->docOutFile_name}</a>], 
                <input type="hidden" name="docfile_name" id="docfile_name" value="{$obj_list_docoutfile[pi]->docOutFile_name}">
                <input type="hidden" name="docfile_path" id="docfile_path" value="{$obj_list_docoutfile[pi]->docOutFile_path}">
            {/section}
        </div>
    ...
    <fieldset style="width:555px; border:1px solid #99bbe8; margin-left:3px; margin-top:5px">
        ...
        <div style="float:left; height:290px; width:555px; overflow-y:scroll">
            ...
            <div style="float:left">
                ...
                <input type="checkbox" name="department_id" id="department_id" value="{$obj_list_department[qi]->department_id}" onClick="tnvb_docheckone();">
                </div>
                ...
                <div class="{$class_td}" style="width:132px; text-align:center">
                    <select name="receive_per" id="receive_per" class="select_middle" style="width:122px">
                        ...
                        {php}
                            $sql="select * from tbl_users where unit_id =".$this->_tpl_vars['obj_list_department'][$this->_sections['qi']['index']]->department_id."";
		...
		<script language="javascript">
            ...
            $("#submitTNVB").click(function() {
                ...
                if (department_id == ""){
                    ...
                    $.post('index.php?addReceiveOut&mod=truyennhans&add_edit=1'+arg,
                        {
                            doc_unit:doc_unit,
                            doc_code:doc_code,
							docout_id:docout_id,
                        ...
                        function(data){
                            ...
                            $("#list_truyenvb_cont").fadeOut('fast').load('index.php?viewReceiveOut&mod=truyennhans'+arg).fadeIn("fast");
