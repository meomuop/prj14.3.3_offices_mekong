...
--------------------------------------------*/
	...
	if (isset($vars['add_edit'])) {
		...
		require_once($CLASSES_TNVB."clsDocFiles.php"); 
		$obj_file = new docoutfile_trans_class();
		$docoutfile_path_arr = explode('|',substr($vars['docoutfile_path'],0,strlen($vars['docoutfile_path'])-1));
		$docoutfile_name_arr = explode('|',substr($vars['docoutfile_name'],0,strlen($vars['docoutfile_name'])-1));
		
		if (count($docoutfile_path_arr)>0 && count($docoutfile_name_arr)>0):
			for($i=0; $i<count($docoutfile_name_arr); $i++):
				if($docoutfile_name_arr[$i]!='0_tmp'):
				$obj_file->addfile($last_doc_id,$docoutfile_name_arr[$i],$docoutfile_path_arr[$i],$_SESSION['user_id']);
				endif;
			endfor;
		endif;
	...
	$obj_doc->getDBbyPkey($vars['docout_id']);
	$doc_info = (array)$obj_doc;
	
	// --- Get doc file list
	$obj_docoutfile		= new docOutFile_class();
	$where_docoutfile 		= " 1 = 1 and docout_id = ".$obj_doc->docOut_id;
	$obj_list_docoutfile	= $obj_docoutfile->getDBList(" $where_docoutfile", "docOutFile_name asc", FALSE, "");
	
	// --- Get doc cat list
	$obj_docCat		= new docCat_class();
	$obj_docCat->getDBbyPkey($obj_doc->docCat_id);
	$docCat_info = (array)$obj_docCat;
	
	// --- Get signper list
	$obj_signper	= new users_class();
	$obj_signper->getDBbyPkey($obj_doc->docOut_signed);
	$signPer_info = (array)$obj_signper;
	...
	$assign_list['docCat_info'] = $docCat_info;
	$assign_list['signPer_info'] = $signPer_info;
	$assign_list['docout_id'] 	= $vars['docout_id'];
	
	$assign_list["error"] = $error;
	$assign_list["errorfile"] = $errorfile;
	
	$display = dirname(__FILE__)."/skin/B_ReceiveOut_add.tpl";
	...
	$smarty->display(dirname(__FILE__)."/skin/B_ReceiveOut_add.tpl");
	else
