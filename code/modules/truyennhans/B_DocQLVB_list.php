...
--------------------------------------------*/
	...
	// --- Condition
	$where = " 1 = 1 and docOut_id not in(select docout_id from tbl_vbs) and docOut_id in(select docOut_id from tbl_docoutfiles)";
	...
	if( in_array($_SESSION["user_level"],array(6,7,8)) ):
	...
	$obj = new docOut_class();
	$obj_list = $obj->getDBList(" $where", "docOut_num DESC", FALSE, $limit);
	...
	$display = dirname(__FILE__)."/skin/B_DocQLVB_tbl.tpl";
	...
	$smarty->display(dirname(__FILE__)."/skin/B_DocQLVB_tbl.tpl");
	else
