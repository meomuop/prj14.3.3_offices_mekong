...
--------------------------------------------*/
 ...
 $where .= " AND receive_time Like '".date('Y-m-d')."%'";
 
 $limit = " LIMIT ".(string)$cur_pos.", ".(int)$vars['numresult'];
 
 $where.=" and (receive_per=".$_SESSION['user_id']." or (R_department_id = ".$_SESSION['access_department_id']." AND receive_per = 0) or receive_id in(select receive_id from tbl_recusers where user_id=".$_SESSION['user_id']."))";
 
 if($order_id == 0):
 $where.="";
 elseif($order_id == 1):
 $where.=" and ((receive_per != 0 and read_status != 1) or(receive_per = 0 and receive_id in(select receive_id from tbl_recusers where read_status != 1)))";
 else:
 $where.=" and ((receive_per != 0 and read_status = 1) or(receive_per = 0 and receive_id in(select receive_id from tbl_recusers where read_status = 1)))";
 endif;
 ...
 $display = dirname(__FILE__)."/skin/B_DocReceive_day_tbl.tpl";
 ...
 if (isset($vars['activeAjax']))
 	$smarty->display(dirname(__FILE__)."/skin/B_DocReceive_day_tbl.tpl");
 else
